...
class JugadorOut(BaseModel):
    datos_pack: dict = Field(default_factory=dict)
...
class NegociarContratoTraspasoIn(BaseModel):
    ...
    ronda: int = 0
    addons: list[AddOnIn] = []
    anios: int = Field(default=3, ge=1, le=5)
    clausula_rescision: int | None = Field(default=None, gt=0)
